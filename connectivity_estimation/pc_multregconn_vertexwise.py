...
def pc_multregconn_vertexwise(activity_matrix, parcel_num, n_components=None):
    """
    ...
    dlabels = np.squeeze(nib.load(defaultdlabelfile).get_data()); # 91282 x 1 (includes cortex + subcortex, first 59412 are cortical vertices), dlabels[:nVertices] would give parcel #s 1-360
    dlabelsCortOnly = dlabels[:nVertices]; 
    ...
    if n_components is not None: 
        ...
        xReg = pca.fit_tranform(sourceData.T); # TRs x n_components
        ...
        pca = PCA(); 
        xReg = pca.fit_tranform(sourceData.T); # if # features (vertices) > # observations (TRs), xReg will be square (TRs x TRs)
