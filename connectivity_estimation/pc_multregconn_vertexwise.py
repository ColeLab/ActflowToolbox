...
def pc_multregconn_vertexwise(activity_matrix, parcel_num, dlabelfile=defaultdlabelfile, n_components=None):
    """
    A function to estimate functional connectivity with principal component regression, vertex-wise and with 10 mm dilated 
    mask exclusion (in source timeseries), as in Cole et al., 2016 [Nat. Neuro.] but with  Glasser (2016) parcels (also: 
    surface vertices instead of voxels) and the locally non-circular modification (10 mm mask). Incorporates code from 
    pc_multregconn.py and calcconn_parcelwise_noncircular_surface.py. The main usage difference is that this will output 
    connectivity estimates in vertex space (also more computationally intensive). 
    
    INPUTS
    activity_matrix: vertices (variables) x time or TRs (observations). EX: resting-state residuals yielded by 
    pre-processing GLM(s). Note: source and target timeseries will be generated by parcel_num input.
    
    parcel_num: specifies the Glasser (2016) parcel to process; scalar value in range of 1-360 (see supplement of 
    2016 paper for parcel list). Whole-cortex results can be obtained by iterating over parcel_num (1-360). 
    
    dlabelfile: parcellation file; each vertex indicates the number corresponding to each parcel. dlabelfile needs 
    to match same vertex dimensions of data
    
    n_components: Optional (but highly recommended for speed in vertex-wise application). Number of PCA components 
    to use. If None, the full set of components will be used (equal to time dimension - 1; for example, number of 
    resting-state TRs in activity_matrix - 1). 
    
    
    OUTPUT
    connectivity_mat: formatted as targets x sources. Main difference from pc_multregconn is that it returns data 
    in vertex space. Target and source sizes will be different for each parcel (each contains a different number 
    of vertices). Note: this will be of size 59412 x 59412, with zeros in rows and columns that are not indexed as 
    target and source vertices (bigger matrix, but allows later concatenation when iterating over whole-cortex; 
    otherwise can remove rows/cols that have zeros and use that data as parcel vertices; average on row dimension 
    to get 1 parcel value for each vertex source)
    
    POTENTIAL TO-DO LIST: 
    - modify output format? currently has zeros in non-parcel vertex locations; maybe return target/source indices 
    as separate outputs (along with FC matrix; non-zero'd) and let user index themselves? 
    - add in a cross validation procedure to optimize number of components (as an alternative to n_components 
    input above); for consistency, use n_comp_search, n_components_min, and n_components_max syntax from pc_multregconn.py 
    - subcortex options & input to set the default dlabel file & directory 
    - a sister script to re-combine whole-cortex results into 1 graph (reorganized back into original vertex order, and/or 
    into CAB-NP order)? 
